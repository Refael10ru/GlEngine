...
namespace GLEngine
{
	...
	public:
		...
		static float* VertexVectorToFloatArray(std::vector<Vertex3Df> pointArray)	
		{
			int size;

			float* floatArray = new float[(size = pointArray.size()) * 3]; 

			for (int x = 0; x < size; x += 3)	
			{
				floatArray[x] = pointArray.at(x).Position.X;
				floatArray[x + 1] = pointArray.at(x).Position.Y;
				floatArray[x + 2] = pointArray.at(x).Position.Z;
			}
